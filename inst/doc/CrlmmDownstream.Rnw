...
<<center>>=
copyNumber(oligoSet) <- VanillaICE:::center(copyNumber(oligoSet), at=2)
@

<<hmmOpts>>=
hmmOpts <- VanillaICE:::newHmmOptionList(snpsetClass=class(oligoSet), verbose=1L)
@

<<hmm, cache=TRUE>>=
fit <- VanillaICE:::hmm2(oligoSet, hmmOpts)
@
...
<<clean, echo=FALSE>>=
showWarnings(rm(oligoSet, fit, res.altered, palette, df)); gc()
@
...
<<hmmCNSet,cache=TRUE>>=
fit2 <- VanillaICE:::hmm2(hapmapSet, hmmOpts, centerCN=TRUE)
@
